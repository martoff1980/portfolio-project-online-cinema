...
async def register(payload: UserCreate, db: AsyncSession = Depends(get_db)):
    ...
    if not user_group:
        # Создаем группу, если вдруг нет в БД (для стабильности при первом запуске)
...
async def activate_account(token: str, db: AsyncSession = Depends(get_db)):
    ...
    user_result = await db.execute(select(User).where(User.id == act_token.user_id))
...
async def resend_activation(payload: PasswordResetRequest, db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(User).where(User.email == payload.email))
    ...
    token_result = await db.execute(select(ActivationToken).where(ActivationToken.user_id == user.id))
...
async def login(payload: UserLogin, db: AsyncSession = Depends(get_db)):
    ...
    if not user.is_active:
        raise HTTPException(status_code=400, detail="Account is not active. Please activate first.")
